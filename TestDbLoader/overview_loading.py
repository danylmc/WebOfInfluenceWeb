...
def load_csv_candidate_donations_17(file_name, year):
    ...
    for index, row in file.iterrows():
        try:
            ...
            ld.import_data(connection, mycursor, 
                        f"{year}_Candidate_Donation_Overview", 
                        ["total_donations", "total_expenses", "people_id", "party_id", "electorate_id", "part_a", "part_b", "part_c", "part_d", "part_f", "part_g", "part_h", "year", "original_id"], 
                        (total_donations, total_expenses, person_id, party_id, electorate_id, part_a, part_b, part_c, part_d, part_f, part_g, part_h, year, original_id))
        except Exception as e:
            print(f"Error processing row {index}: {e}")
...
def load_csv_candidate_donations_14(file_name, year):
    ...
    for index, row in file.iterrows():
        try:
            ...
            ld.import_data(connection, mycursor, 
                           f"{year}_Candidate_Donation_Overview", ["total_donations", "total_expenses", "people_id", "party_id", "electorate_id", "part_a", "part_b", "part_c", "part_d", "year", "original_id"], 
                           (total_donations, total_expenses, person_id, party_id, electorate_id, part_a, part_b, part_c, part_d, year, original_id))
        except Exception as e:
            print(f"Error processing row {index}: {e}")
